...
for indi in indices:
	...
	print(id_name)
	df_id=pd.read_csv('D:\paper_data\\00_indices\\03_time_res\\17_6_1\SWI_indices_%s'%(id_name),encoding='gb18030')
	df_CSAD = pd.read_csv('D:\paper_data\\02_time_res\\17_6_1\\02_processed_data\%s'%(indi),encoding='gb18030')
	...
	for i in range(2,cols):
		...
		if indi=='CSAD_Whole.csv':
			path='D:/paper_data/02_time_res/17_6_1/04_regression/Whole'
			save_txt(result,name,path)
			std_output(res,name,path)
		if indi=='CSAD_Pre.csv':
			path='D:/paper_data/02_time_res/17_6_1/04_regression/Pre'
			save_txt(result,name,path)
			std_output(res,name,path)

		if indi=='CSAD_Post.csv':
			path='D:/paper_data/02_time_res/17_6_1/04_regression/Post'
			save_txt(result,name,path)
			std_output(res,name,path)
